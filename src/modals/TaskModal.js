...
export default function TaskModal({ taskIndex, colIndex, setIsTaskModalOpen}) {
  ...
  console.log('this is what I work with', subtasks);
  ...
  return (
    ...
    >
      <div className={`task-modal ${isDeleteModalOpen && "none"}`}>
        ...
        {subtasks.filter(subtask => subtask.parent == null).map((subtask, index) => {
          return (
            <div>  
              <Subtask
                index={index}
              ...
              />
              {subtasks.filter(insideSubtask => insideSubtask.parent === subtask.id).map((insideSubtask, i) => {
                return (
                  <div className="subtask-line" key={i}>  
                      <img
                        src={subtaskIcon}
                        alt="icon-down-right"
